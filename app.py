...
SUPABASE_KEY = '...'  # reemplaza por tu anon key
SUPABASE_SERVICE_KEY = '...'  # reemplaza por tu service_role key
SUPABASE_HEADERS = {
    "apikey": SUPABASE_KEY,
...
def dashboard():
    ...
    if usuario == 'admin':
        doctoras = requests.get(f"{SUPABASE_URL}/rest/v1/doctoras", headers=SUPABASE_HEADERS).json()
        establecimientos = requests.get(f"{SUPABASE_URL}/rest/v1/establecimientos", headers=SUPABASE_HEADERS).json()
    ...
    return render_template('dashboard.html', usuario=usuario, eventos=eventos,
                           doctoras=doctoras, establecimientos=establecimientos,
                           formularios=formularios, conteo=conteo)
    ...
    for archivo in archivos:
        if permitido(archivo.filename):
            ...
            if res_upload.status_code in [200, 201]:
                ...
                res_insert = requests.post(f"{SUPABASE_URL}/rest/v1/formularios_subidos",
...
def evaluados(establecimiento):
    ...
    usuario = session['usuario']
    res_est = requests.get(f"{SUPABASE_URL}/rest/v1/establecimientos?id=eq.{establecimiento}&select=nombre",
