...
def dashboard():
    ...
    try:
        ...
        # Obtener datos para el gráfico de rendimiento por doctora
        doctoras_url = f"{SUPABASE_URL}/rest/v1/usuarios?select=id,username,establecimiento"
        doctoras_res = requests.get(doctoras_url, headers=SUPABASE_SERVICE_HEADERS)
        doctoras_res.raise_for_status()
        doctoras_data = doctoras_res.json()

        doctoras_map = {doc['id']: doc['username'] for doc in doctoras_data}
        doctor_evaluations = {}

        for doc_id, doc_name in doctoras_map.items():
            # Solo contar formularios completados por doctoras del mismo establecimiento
            if any(doc['id'] == doc_id and doc['establecimiento'] == establecimiento for doc in doctoras_data):
                evaluaciones_url = f"{SUPABASE_URL}/rest/v1/estudiantes_nomina?doctora_evaluadora_id=eq.{doc_id}&fecha_relleno=not.is.null&select=count"
                evaluaciones_res = requests.get(evaluaciones_url, headers=SUPABASE_SERVICE_HEADERS)
                evaluaciones_res.raise_for_status()
                count_data = evaluaciones_res.json()
                count = count_data[0]['count'] if count_data else 0
                doctor_evaluations[doc_id] = count
        ...
        return render_template('dashboard-7.html', 
                               ...
                               doctoras_map=doctoras_map) # Pass the map to HTML for display names
...
def crear_nomina():
    ...
    tipo_nomina_val = request.form['tipo_nomina'].strip() # Get and strip spaces
    form_type_val = request.form.get('form_type') # Get with .get() to avoid KeyError if not present
    establecimiento = session.get('establecimiento')
    user_id = session.get('user_id')

    # Safeguard: If form_type is not explicitly set by JS, derive it from tipo_nomina
    if not form_type_val: # This handles cases where JS didn't set it, or it was empty
        if tipo_nomina_val == 'Neurología':
            form_type_val = 'neurologia'
        elif tipo_nomina_val == 'FAMILIAR':
            form_type_val = 'medicina_familiar'
        else:
            # If still not determined, set to None or a default that will trigger an error
            form_type_val = None 
    
    if not nombre_nomina or not tipo_nomina_val or not form_type_val or not establecimiento or not user_id:
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            if estudiantes_a_insertar:
                print(f"DEBUG: Intentando insertar {len(estudiantes_a_insertar)} estudiantes a Supabase.")
                res = requests.post(f"{SUPABASE_URL}/rest/v1/estudiantes_nomina", headers=SUPABASE_SERVICE_HEADERS, json=estudiantes_a_insertar)
                res.raise_for_status() # This will raise an exception for 4xx or 5xx responses
            ...
            # Log the full response text from Supabase for detailed error
            error_message = f"Error al comunicarse con Supabase: {e}"
            if e.response is not None:
                error_message += f" Respuesta de Supabase: {e.response.text}"
            print(f"ERROR: {error_message}")
            flash(f"Error al guardar los estudiantes en la base de datos: {error_message}", "error")
        except Exception as e:
            print(f"ERROR: Error al procesar el archivo o subir estudiantes: {e}")
                    ...
                    if field_name:
                        ...
                        # Campos específicos de Medicina Familiar
                            ...
                            # Visión (checkboxes)
                                ...
                                writer.update_page_form_field_values(writer.pages[page_num], {"VICIOS DE REFRACCION": "/Yes"})
                        ...
                        if field_name:
                            ...
                            # Campos específicos de Medicina Familiar
                                ...
                                # Visión (checkboxes)
                                    ...
                                    combined_writer.update_page_form_field_values(new_page, {"VICIOS DE REFRACCION": "/Yes"})
