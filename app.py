                    ...
                    if field_name:
                        ...
                        # Campos específicos de Medicina Familiar
                            ...
                            # Fechas
                                ...
                                # Usar el campo fecha_reevaluacion_pdf que se guarda en formato DD/MM/YYYY
                                writer.update_page_form_field_values(writer.pages[page_num], {"Fecha reevaluación": request.form.get('fecha_reevaluacion_pdf', '')})

                            # Observaciones (OBS1 a OBS7)
                            for i in range(1, 8):
                                obs_field_name = f"OBS{i}"
                                if field_name == obs_field_name:
                                    writer.update_page_form_field_values(writer.pages[page_num], {obs_field_name: est.get(f'observacion_{i}', '')})
                            ...
                            elif field_name == "Salud pública" and est.get('procedencia_salud_publica'): # Asumiendo campo en DB
                                writer.update_page_form_field_values(writer.pages[page_num], {"Salud pública": "/Yes"})
                            elif field_name == "Particular" and est.get('procedencia_particular'): # Asumiendo campo en DB
                                writer.update_page_form_field_values(writer.pages[page_num], {"Particular": "/Yes"})
                            elif field_name == "Escuela" and est.get('procedencia_escuela'): # Asumiendo campo en DB
                                writer.update_page_form_field_values(writer.pages[page_num], {"Escuela": "/Yes"})
                            elif field_name == "Otro" and est.get('procedencia_otro'): # Asumiendo campo en DB
                        ...
                        if field_name:
                            ...
                            # Campos específicos de Medicina Familiar
                                ...
                                for i in range(1, 8):
                                    obs_field_name = f"OBS{i}"
                                    if field_name == obs_field_name:
                                        combined_writer.update_page_form_field_values(new_page, {obs_field_name: est.get(f'observacion_{i}', '')})
                    ...
                    if field_name:
                        ...
                        # Campos específicos de Medicina Familiar
                            ...
                            # Información del profesional (se asume que se rellena con datos de la doctora logeada)
                                ...
                                writer.update_page_form_field_values(new_page, {"Salud pública": "/Yes"})
                            elif field_name == "Particular" and est.get('procedencia_particular'):
                                writer.update_page_form_field_values(new_page, {"Particular": "/Yes"})
                            elif field_name == "Escuela" and est.get('procedencia_escuela'):
                                combined_writer.update_page_form_field_values(new_page, {"Escuela": "/Yes"})
                            elif field_name == "Otro" and est.get('procedencia_otro'):
                                combined_writer.update_page_form_field_values(new_page, {"Otro": "/Yes"})
